...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [statsRes, atsRes, skillsRes] = await Promise.all([
          adminService.getStats(),
          adminService.getAtsDistribution(),
          adminService.getSkillsDemand()
        ]);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 p-8 max-w-7xl mx-auto w-full pt-24 pb-12">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Recruiter Analytics</h1>
          <p className="text-muted-foreground">
            Platform-wide insights on candidate quality, skill gaps, and placement funnels.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border">
            <CardBody className="p-6">
              <h3 className="text-lg font-semibold mb-6">ATS Score Distribution</h3>
              <div className="h-80">
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border">
            <CardBody className="p-6">
              <h3 className="text-lg font-semibold mb-6">Top Skills: Demand vs Supply</h3>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={skillsData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#333" vertical={false} />
                    ...
                    />
                    <Legend />
                    <Line type="monotone" dataKey="demand" stroke="#6366f1" strokeWidth={2} name="Demanded by Companies" dot={{ r: 4 }} activeDot={{ r: 6 }} />
                    <Line type="monotone" dataKey="supply" stroke="#10b981" strokeWidth={2} name="Possessed by Students" dot={{ r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </CardBody>
